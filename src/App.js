  ...
  return <div className="site-shell">
    <header className={`nav ${scrolled ? 'nav--scrolled' : ''}`}>
      ...
      <nav className={menuOpen ? 'nav-links nav-links--open' : 'nav-links'} aria-label="Päävalikko">
        <a href="#palvelut" onClick={closeMenu}>Palvelut</a><a href="#smartest" onClick={closeMenu}>SmarTEST</a><a href="#blogi" onClick={closeMenu}>Blogi</a><a href="#referenssit" onClick={closeMenu}>Referenssit</a><a className="nav-cta" href="#yhteys" onClick={closeMenu}>Ota yhteyttä</a>
      </nav>
    </header>
    <main>
      ...
      <section className="blog section-pad" id="blogi"><div className="section-heading"><div><p className="section-kicker">Blogi</p><h2>Ajatuksia<br />tuotekehityksestä.</h2></div><p>Kokemuksia, havaintoja ja käytännön tietoa ohjelmistoista, elektroniikasta ja sulautetuista järjestelmistä.</p></div>{posts.length > 0 ? <div className="blog-grid">{posts.map((post) => <a className="blog-card" href={`#blog/${post.slug}`} key={post.id || post.slug}><div className="blog-image">{post.image ? <img src={`${process.env.PUBLIC_URL}${post.image}`} alt="" /> : <span>FLOUN / BLOGI</span>}</div><time dateTime={post.date}>{new Date(post.date).toLocaleDateString('fi-FI')}</time><h3>{post.title}</h3><p>{post.excerpt}</p><span className="read-more">Lue artikkeli ↗</span></a>)}</div> : <div className="blog-empty"><span>FLOUN / BLOGI</span><div><h3>Ensimmäinen kirjoitus on työn alla.</h3><p>Tänne kokoamme pian käytännön havaintoja teknologiasta ja tuotekehityksestä.</p></div></div>}</section>
